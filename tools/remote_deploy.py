...
def deploy(hostname, password):
    ...
    # Check if dir exists
    check_dir = client.exec_command(f"[ -d {repo_dir} ] && echo 'exists'")[1].read().decode().strip()
    ...
    if check_dir == "exists":
        ...
        run_command(client, f"git clone https://github.com/yunaamelia/debian-vps-workstation.git {repo_dir}")
