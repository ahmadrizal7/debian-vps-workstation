...
class TestOhMyZshInstallation:
    ...
    def test_install_oh_my_zsh_for_all_users(self, mock_run, mock_pwd, module):
        ...
        mock_run.return_value = Mock(success=True, stdout="")
    ...
    @patch('configurator.modules.desktop.pwd')
    @patch.object(DesktopModule, 'run')
    def test_oh_my_zsh_skips_existing_installations(self, mock_run, mock_pwd, module):
        ...
        with patch('configurator.modules.desktop.os.path.exists', side_effect=lambda path: '.oh-my-zsh' in path or False):
    ...
    @patch('configurator.modules.desktop.pwd')
    @patch.object(DesktopModule, 'run')
    def test_oh_my_zsh_registers_rollback(self, mock_run, mock_pwd, module):
        ...
        assert any('.oh-my-zsh' in call for call in rollback_calls)
...
class TestPluginInstallation:
    ...
    @patch('configurator.modules.desktop.pwd')
    @patch.object(DesktopModule, 'run')
    def test_install_zsh_plugins_installs_autosuggestions(self, mock_run, mock_pwd, module):
        ...
        assert any('zsh-autosuggestions' in call for call in git_clones)
    
    @patch('configurator.modules.desktop.pwd')
    @patch.object(DesktopModule, 'run')
    def test_install_zsh_plugins_installs_syntax_highlighting(self, mock_run, mock_pwd, module):
        ...
        git_clones = [str(c) for c in mock_run.call_args_list if 'git clone' in str(c)]
        assert any('zsh-syntax-highlighting' in call for call in git_clones)
...
class TestTerminalTools:
    ...
    def test_install_terminal_tools_installs_all_tools(self, mock_install_pkg, module):
        ...
        all_packages = []
        for call in mock_install_pkg.call_args_list:
            all_packages.extend(call[0][0])
        
        required_tools = ["fzf", "bat", "exa", "zoxide"]
        for tool in required_tools:
            # Note: package names might differ based on OS functionality
            # Assuming 'fzf', 'bat', 'exa', 'zoxide' are the package names or in the list
...
class TestDefaultShellConfiguration:
    ...
    @patch('configurator.modules.desktop.pwd')
    @patch.object(DesktopModule, 'run')
    def test_set_zsh_as_default_shell_changes_shell(self, mock_run, mock_pwd, module):
        ...
        # Verify Zsh path
        assert any('/usr/bin/zsh' in call for call in chsh_calls)
    
    @patch('configurator.modules.desktop.pwd')
    @patch.object(DesktopModule, 'run')
    def test_set_zsh_skips_if_already_default(self, mock_run, mock_pwd, module):
