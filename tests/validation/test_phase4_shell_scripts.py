...
class TestZshrcSyntaxValidation:
    ...
    def test_zshrc_contains_required_sections(self, module):
        ...
        for section in required_sections:
            assert section in zshrc_content, \
                f"Required section missing from .zshrc: {section}"
    ...
    def test_syntax_highlighting_plugin_is_last(self, module):
        ...
        plugin_list = [l for l in plugin_lines if l and not l.startswith('#') and l != ')' and 'plugins=(' not in l]
    ...
    def test_aliases_are_safe(self, module):
        ...
        for pattern in dangerous_patterns:
            assert pattern not in zshrc_content, \
                f"Dangerous alias pattern found: {pattern}"
    ...
    def test_environment_variables_safe(self, module):
        ...
        assert "export EDITOR='vim'" in zshrc_content or \
               "export EDITOR='nano'" in zshrc_content or \
               "export EDITOR=" not in zshrc_content, \
    ...
    def test_conditional_tool_loading(self, module):
        ...
        if 'fzf' in zshrc_content:
            assert 'command -v fzf' in zshrc_content or \
                   'if command -v fzf' in zshrc_content or \
                   'if [ -f' in zshrc_content, \
                "FZF integration should check if fzf is installed"
        
        # Zoxide integration should be conditional
        if 'zoxide' in zshrc_content:
            assert 'command -v zoxide' in zshrc_content or \
                   'if command -v zoxide' in zshrc_content, \
...
class TestPowerlevel10kConfiguration:
    ...
    def test_p10k_config_is_valid_zsh_syntax(self, module):
        ...
        try:
            ...
            try:
                ...
                assert result.returncode == 0, \
                    f"Zsh syntax error in .p10k.zsh:\n{result.stderr}"
            except FileNotFoundError:
                pytest.skip("zsh not installed in test environment")
    ...
    def test_p10k_instant_prompt_enabled(self, module):
        ...
        assert 'POWERLEVEL9K_INSTANT_PROMPT=verbose' in p10k_content or \
               'POWERLEVEL9K_INSTANT_PROMPT=quiet' in p10k_content
    ...
    def test_zshrc_sources_p10k_config(self, module):
        ...
        assert 'source ~/.p10k.zsh' in zshrc_content or \
               '[[ ! -f ~/.p10k.zsh ]] || source ~/.p10k.zsh' in zshrc_content
