...
from configurator.utils.file import write_file, backup_file
...
class SystemModule(ConfigurationModule):
    ...
    def verify(self) -> bool:
        ...
        if current_tz != expected_tz:
            self.logger.warning(
                f"Timezone mismatch: expected {expected_tz}, got {current_tz}"
            )
            checks_passed = False
...
"""
    ...
    def _configure_swap(self):
        ...
        if free_gb < swap_size_gb + 5:  # Keep at least 5GB free
            self.logger.warning(
                f"Skipping swap creation: only {free_gb:.1f} GB free"
            )
            return
