...
class WireGuardModule(ConfigurationModule):
    ...
    def _configure_server(self):
        ...
        server_private = self.state.get("server_private") or \
            Path("/etc/wireguard/server_private.key").read_text().strip()
...
"""
    ...
    def _generate_client_config(self):
        ...
        # Get server info
        server_public = self.state.get("server_public") or \
            Path("/etc/wireguard/server_public.key").read_text().strip()
        port = self.state.get("port", 51820)
...
"""
        ...
        self.logger.info(f"  Add peer to server: wg set wg0 peer {client_public} allowed-ips 10.0.0.2/32")
